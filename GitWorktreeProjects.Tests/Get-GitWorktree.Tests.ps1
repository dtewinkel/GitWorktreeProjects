...
Describe "Get-GitWorktree" {
	...
	It "should return all worktrees for the currenct project if no parameters are given" {
		...
		Should -Invoke GetProjectConfig -ModuleName GitWorktreeProjects -ParameterFilter { $Project -eq '.' -and $WorktreeFilter -eq '*' }
	...
	It "should return all worktrees for the project and worktrees for which the parameters are given" {
		...
		Should -Invoke GetProjects -ModuleName GitWorktreeProjects -ParameterFilter { $Filter -eq $projectFilter }
		Should -Invoke GetProjectConfig -ModuleName GitWorktreeProjects -ParameterFilter { $Project -eq $projectFilter -and $WorktreeFilter -eq $WorktreeFilter }
		$worktrees | Should -HaveCount 2
		$worktrees[0].Name | Should -Be "Test123"
		$worktrees[1].Name | Should -Be "Demo123"
	}
	...
	It "should fail if -Project is not '.' and no project can be found" {
		...
		Should -Invoke GetProjects -ModuleName GitWorktreeProjects -ParameterFilter { $Filter -eq $projectName } -Times 1
